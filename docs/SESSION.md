...
2026-07-01 | (docs) | Patch #008 | main | Docs | Moved `SESSION.md`, `HANDOFF.md`, `ARCHITECTURE.md`, `DECISIONS.md`, `ROADMAP.md` into `docs/` (enterprise layout: root keeps only `README.md`/`AGENTS.md`/`CLAUDE.md`); reduced `CLAUDE.md` to a ~12-line shim pointing to `AGENTS.md`; turned `AGENTS.md` into a universal multi-agent router; fixed root `README.md` doc links (previously pointed to nonexistent `docs/agents/...` paths) | Regression: NO | Pure relocation + link fixes via `git mv`; no content logic changed. | Emanuele
